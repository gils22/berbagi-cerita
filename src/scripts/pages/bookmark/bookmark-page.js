...
export default class BookmarkPage {
    ...
    const html = reports.reduce((accumulator, report) => {
      return accumulator.concat(`
        <div class="bg-white p-4 rounded-lg shadow-md flex flex-col items-center justify-center">
          <img src="${report.photoUrl || 'https://via.placeholder.com/150'}" alt="Story Photo" class="w-full h-48 object-cover rounded-lg mb-4">
          <h3 class="text-lg font-semibold">${report.name}</h3>
          <p class="text-gray-600 text-sm">${report.description.length > 100 ? report.description.slice(0, 100) + '...' : report.description}</p>
          <p class="text-sm text-gray-500 mt-2">Lokasi: ${report.lat && report.lon ? 'Tersedia' : 'Tidak tersedia'}</p>
          <div class="mt-4 flex space-x-4">
            <button class="btn-detail text-green-600 hover:text-green-800" data-id="${report.id}">
              <i class="fas fa-eye"></i> Detail
            </button>
            <button class="btn-remove text-yellow-600 hover:text-yellow-800" data-id="${report.id}">
  ...
  populateBookmarkedReportsListEmpty() {
    document.getElementById('reports-list').innerHTML = 'Tidak ada cerita yang tersimpan.';
  }

  populateBookmarkedReportsError(message) {
    document.getElementById('reports-list').innerHTML = `<p class="text-red-600">Error: ${message}</p>`;
  }

  showReportsListLoading() {
    document.getElementById('reports-list-loading-container').innerHTML = 'Loading...';
  }
